pragma solidity ^0.8.24;

import {InstanceController} from "../src/InstanceController.sol";
import {FoundryVm} from "./FoundryVm.sol";

/// @notice Start a reporter authority transfer (2-step).
/// @dev Must be executed by `rootAuthority`.
///
/// Env:
/// - `PRIVATE_KEY`
/// - `BLACKCAT_INSTANCE_CONTROLLER`
/// - `BLACKCAT_NEW_REPORTER_AUTHORITY`
contract StartReporterAuthorityTransfer {
    FoundryVm internal constant vm = FoundryVm(address(uint160(uint256(keccak256("hevm cheat code")))));

    function run() external {
        uint256 pk = vm.envUint("PRIVATE_KEY");
        address controller = vm.envAddress("BLACKCAT_INSTANCE_CONTROLLER");
        address newAuthority = vm.envAddress("BLACKCAT_NEW_REPORTER_AUTHORITY");

        vm.startBroadcast(pk);
        InstanceController(controller).startReporterAuthorityTransfer(newAuthority);
        vm.stopBroadcast();
    }
}

