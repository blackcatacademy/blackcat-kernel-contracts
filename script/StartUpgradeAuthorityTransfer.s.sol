...
/// @notice Start an upgrade authority transfer (2-step).
/// @dev Must be executed by `rootAuthority`.
///
/// Env:
/// - `PRIVATE_KEY`
/// - `BLACKCAT_INSTANCE_CONTROLLER`
/// - `BLACKCAT_NEW_UPGRADE_AUTHORITY`
contract StartUpgradeAuthorityTransfer {
    ...
    function run() external {
        ...
        address newAuthority = vm.envAddress("BLACKCAT_NEW_UPGRADE_AUTHORITY");

        vm.startBroadcast(pk);
        InstanceController(controller).startUpgradeAuthorityTransfer(newAuthority);
        vm.stopBroadcast();
    }
}
